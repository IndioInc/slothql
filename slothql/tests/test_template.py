...
def test_resolve_path(template_file):
    ...
    with mock.patch("slothql.config.BASE_DIR", base_dir):
        with mock.patch(
            "slothql.config.TEMPLATE_DIRS", [os.path.basename(template_dir)]
        ):
            path = template.resolve_path("file.html")
    assert (
        path == f"{base_dir}/{os.path.basename(template_dir)}/file.html"
...
def test_get_template_string(template_file):
    with mock.patch.object(
        template, "resolve_path", return_value=template_file
    ) as resolve_path:
