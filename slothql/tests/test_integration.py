...
def test_nested_model_query():
    ...
    assert {"data": {"b": {"a": {"field": "resolved"}}}} == slothql.gql(
...
@pytest.mark.parametrize(
    "resolver, expected", ((lambda obj, info: [1, 2, 3], [1, 2, 3]), (None, None))
)
...
def test_filtering(query, expected):
    ...
    class Query(slothql.Object):
        foos = slothql.Field(
            Foo, resolver=lambda: [{"id": "1"}, {"id": "2"}, {"id": "3"}], many=True
        )
...
def test_invalid_filter():
    ...
    error = {
        "message": 'Argument "filter" has invalid value {id: {wtf: 1}}.\n'
        'In field "id": Expected type "ID", found {wtf: 1}.'
    }
    assert [error] == slothql.gql(
        slothql.Schema(query=Query), "query { foos(filter: {id: {wtf: 1}}) { id } }"
    ).errors
