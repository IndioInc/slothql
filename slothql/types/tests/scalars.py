...
def test_scalar_integration():
    ...
    assert {"data": {"foo": data}} == slothql.gql(
        schema, "query { foo { integer boolean float id string } }"
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (scalars.StringType, "\U0001F601", "\U0001F601"),
...
def test_serialize__invalid(scalar_type: Type[scalars.ScalarType], value):
    ...
    assert (
        f"`{scalar_type.__name__}.serialize` received invalid value {repr(value)}"
        == str(exc_info.value)
    )
