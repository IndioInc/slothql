...
def test_resolve_field(obj, expected, info_mock, type_mock):
    for field_name, expected_value in expected.items():
        assert expected_value == slothql.Field(type_mock()).resolve_field(
            obj, info_mock(field_name=field_name), {}
        )
...
def test_field_comparison_not_field(type_mock):
    ...
    assert (
        "slothql.Field can only be compared with other slothql.Field instances,"
