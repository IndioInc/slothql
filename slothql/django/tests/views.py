...
@pytest.mark.parametrize('value', (
    ...
    rf.post(path='', data={'query': 'query query1 { hello } query query2 { hello }', 'operationName': 'query1'}),
))
...
def test_show_graphiql(content_type, hello_schema):
    ...
    with mock.patch.object(GraphQLView, 'template', render=lambda *_: '<html>hello world</html>'):
        response = GraphQLView.as_view(schema=hello_schema)(request)
    assert (200, b'<html>hello world</html>') == (response.status_code, response.content)
