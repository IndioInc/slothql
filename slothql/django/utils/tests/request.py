...
class TestGetOperation:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                rf.post("", data=data, content_type="text/html"),
                f"Unsupported content-type text/html",
            ),
        ),
    )
    ...
    @mock.patch(
        "slothql.operation.Operation.from_raw_json",
        side_effect=InvalidOperation("mocked exception"),
    )
    def test_query_from_request__post_invalid_json(self, from_raw_json):
        request = self.rf.post(
            "", data='{"query": "hello"}', content_type="application/json"
        )
        with pytest.raises(ValidationError) as exc_info:
